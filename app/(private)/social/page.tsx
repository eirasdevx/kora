...
export default function SocialPage() {
  ...
  const filteredPosts = useMemo(() => {
    ...
    return posts.filter((post) => {
      ...
      const matchesFilter =
        activeFilter === "Todo" ||
        post.channels.includes(activeFilter);
      ...
      <section className="space-y-4">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex items-center gap-2 text-sm text-gray-500">
            ...
            <button className="inline-flex items-center gap-2 rounded-xl border border-gray-200 bg-white px-4 py-2 text-sm font-semibold text-gray-600 shadow-sm">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {filteredPosts.map((post, index) => (
            ...
            >
              <div
                className={`relative h-40 ${GRADIENTS[index % GRADIENTS.length]}`}
              >
                <div className="absolute left-3 top-3 flex gap-2">
                  <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-white/80 text-sm font-semibold text-primary">
                    IG
                  </span>
                  <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-white/80 text-sm font-semibold text-blue-600">
                    FB
                  </span>
                </div>
              </div>
              <div className="space-y-2 px-4 py-4">
                <div className="flex items-center gap-2 text-xs text-gray-500">
                  <span
                    className={`rounded-full px-2.5 py-1 text-[11px] font-semibold ${STATUS_STYLES[post.status]}`}
                  >
              ...
              <div className="flex items-center justify-between border-t border-gray-100 px-4 py-3 text-xs text-gray-500">
                <div className="flex -space-x-2">
                  {["A", "L"].map((letter) => (
                    <span
                      key={letter}
                      className="flex h-7 w-7 items-center justify-center rounded-full border border-white bg-primary/10 text-xs font-semibold text-primary"
                    >
                      {letter}
                    </span>
                  ))}
          ...
          <div className="flex items-center gap-4">
            <span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-blue-50 text-blue-600">
              ...
              >
                ...
                <path d="M14.5 10.5l2.5-2.5" />
                <path d="M9.5 13.5L7 16" />
                <path d="M14.5 13.5l2.5 2.5" />
                <path d="M9.5 10.5L7 8" />
              </svg>
            </span>
            <div>
              ...
              <p className="text-sm text-gray-500">
                Gestionas 3 redes sociales activamente.
              </p>
            </div>
          </div>

          <div className="flex flex-wrap items-center gap-2">
            {["Facebook", "Instagram", "X (Twitter)"].map(
              (channel) => (
                <span
                  key={channel}
            ...
            <button className="rounded-xl bg-primary/10 px-4 py-2 text-xs font-semibold text-primary">
              Gestionar canales
            </button>
          </div>
        </div>
